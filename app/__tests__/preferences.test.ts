...
const client = (preferences?: { showWishlistInSearch: boolean }) =>
  new MatchClient({
    catalog: makeCatalog(),
...
describe("per-user wishlist-in-search control (section 4.16)", () => {
  ...
  it("returns nothing at all once the user opts out", async () => {
    const result = await client({ showWishlistInSearch: false }).requestMatch(request, true);
  ...
  it("is enforced in the service, not by the UI declining to draw", async () => {
    ...
    const subject = client({ showWishlistInSearch: false });
  ...
  it("is indistinguishable from a genuine miss", async () => {
    const optedOut = await client({ showWishlistInSearch: false }).requestMatch(request, true);
